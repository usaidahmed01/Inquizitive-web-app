...
import { normalizeFullName } from "@/schemas";
...
export default function JoinClassPage() {
    ...
    (async () => {
      try {
        ...
        if (r.ok) {
          ...
          setCourseName(String(j?.course_name || "Unknown"));
        } else {
          setCourseName("Unknown");
        }
      } catch {
        if (alive) setCourseName("Unknown");
      }
    })();
    return () => {
      alive = false;
  ...
  const onSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const r = await fetch(`${API}/public/join/${classId}`, {
        method: "POST",
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-5 md:mb-6">
            ...
            <h1 className="text-xl md:text-2xl font-extrabold mt-1 text-[#2B2D42]">
              Enter Your Details
            </h1>
              ...
              {!seatCheck.success && seat.length > 0 && (
                <p className="mt-1 text-xs text-red-600">
                  {seatCheck.error.issues[0]?.message ||
                    "Seat No must be B followed by 11 digits."}
                </p>
              )}
            </div>
...
function SuccessSplash() {
  const pieces = Array.from({ length: 24 }).map((_, i) => ({
    ...
    c: ["#2E5EAA", "#81B29A", "#4A8FE7", "#F2A541", "#7ED0B6"][
