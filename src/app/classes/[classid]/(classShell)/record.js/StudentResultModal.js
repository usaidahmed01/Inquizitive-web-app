...
export default function StudentResultModal({
  ...
  results,
  error,
  onEditStudent, // (payload) => void  ← parent wraps seat handling
  onDeleteStudent, // (idLike) => void (seat in this flow)
}) {
  const [editOpen, setEditOpen] = useState(false);
  const [delOpen, setDelOpen] = useState(false);
  ...
  useEffect(() => {
    const anyOpen = open || editOpen || delOpen;
    if (!anyOpen) return;
    const prev = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    return () => { document.body.style.overflow = prev; };
  }, [open, editOpen, delOpen]);
        ...
        {open && (
          <Portal>
              ...
              >
                ...
                <div className="p-6 bg-white max-h-[60vh] overflow-y-auto space-y-4">
                  {error ? (
                    ...
                    <div className="text-sm text-gray-600">Preparing results…</div>
                  ) : !Array.isArray(results.quizzes) || results.quizzes.length === 0 ? (
                    <NoQuizEmpty />
                  ) : (
                    results.quizzes.map((r) => {
            ...
            <ConfirmModal
              ...
              confirmText="Delete"
              onConfirm={async () => {
                await onDeleteStudent?.(s.seat || s.id);
                setDelOpen(false);
                onClose(); // close parent modal too
              }}
            />
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === "Enter" && canSave) {
        onSave?.({ name: formatName(name), seat, email });
      }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [open, name, seat, email, canSave, onClose, onSave]);
        ...
        <>
          ...
          >
            ...
            >
              <div className="px-6 pt-6">
                <h3 className="text-lg font-semibold text-[#2B2D42]">Edit Student</h3>
                <p className="text-sm text-gray-600">Update name, seat number, or email.</p>
              </div>
              <div className="px-6 py-4 space-y-4">
                <div>
                  <label className="block text-xs text-gray-600 mb-1">Full name</label>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full h-11 rounded-lg border px-3 outline-none focus:ring-2 border-gray-300/70 focus:ring-[#2E5EAA]/30"
                  />
                </div>
              </div>
              <div className="px-6 pb-6 pt-2 flex items-center justify-end gap-3">
