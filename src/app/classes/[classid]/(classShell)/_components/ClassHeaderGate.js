...
export default function ClassHeaderGate() {
    ...
    const top = segments?.[0];     // null (record) or "quiz"
    const sub = segments?.[1];     // "generate", "[id]", etc.

    
    
    // Show header on Record (null) and Quiz list (top === "quiz" && no sub)
    // const showHeader = !top || (top === "quiz" && !sub);
    const showHeader =
    segments.length === 0 || // record root
    (segments[0] === "quiz" && segments.length === 1); // quiz list only
    
    console.log("segments:", segments, "top:", top, "sub:", sub);
